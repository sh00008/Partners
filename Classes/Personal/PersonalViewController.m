    ...
    box.onTap = ^{
       ...
       /*
        ...
        if (![self photoBoxWithTag:-1] && [self randomMissingPhoto]) {
            [section.boxes addObject:self.libAddBox];
        }
...
- (MGBox *)libAddBox {
    ...
    // make the box
    PersonalLibary *box = [PersonalLibary libAddBoxWithSize:[self libBoxSize]];
    ...
    box.onTap = ^{
        ...
        [photosGrid.boxes insertObject:[self libBoxFor:photo] atIndex:idx];
        [photosGrid layout];
...
- (void)loadIntroSection {
    ...
    // header line
    MGLine *header = [MGLine lineWithLeft:PERSONAL_INFO right:nil size:ROW_SIZE];
