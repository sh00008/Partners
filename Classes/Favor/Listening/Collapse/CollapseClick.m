...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        ...
        CollapseClickCell *cell = [CollapseClickCell newCollapseClickCellWithHeader:[CollapseClickDelegate viewForCollapseClickAtIndex:xx] index:xx content:[CollapseClickDelegate viewForCollapseClickContentViewAtIndex:xx]];
        ...
        cell.ContentView.frame = CGRectMake(0, cell.TitleView.frame.size.height + kCCPad, self.frame.size.width, cell.ContentView.frame.size.height);
        
        // Set cell's size
        cell.frame = CGRectMake(0, totalHeight, self.frame.size.width, cell.TitleView.frame.size.height);
        ...
        totalHeight += cell.TitleView.frame.size.height + kCCPad;
    }
...
-(void)closeCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] == YES) {
        ...
        [UIView animateWithDuration:duration animations:^{
            // Resize Cell
            CollapseClickCell *cell = [self.dataArray objectAtIndex:index];
            cell.frame = CGRectMake(cell.frame.origin.x, cell.frame.origin.y, cell.frame.size.width, cell.TitleView.frame.size.height);
