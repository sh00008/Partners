...
void Base64_EncodeUpdate(ENCODE_CTX *ctx, unsigned char *out, int *outl,
	...
	*outl=0;
	...
	if (ctx->num != 0)
		...
		in+=i;
		inl-=i;
		j=Base64_EncodeBlock(out,ctx->enc_data,ctx->length);
		ctx->num=0;
		out+=j;
		*(out++)='\n';
		*out='\0';
	...
	while (inl >= ctx->length)
		{
		j=Base64_EncodeBlock(out,in,ctx->length);
		in+=ctx->length;
		inl-=ctx->length;
		out+=j;
		*(out++)='\n';
...
void Base64_EncodeFinal(ENCODE_CTX *ctx, unsigned char *out, int *outl)
	...
	if (ctx->num != 0)
		{
		ret=Base64_EncodeBlock(out,ctx->enc_data,ctx->num);
		out[ret++]='\n';
		out[ret]='\0';
		ctx->num=0;
		}
...
int Base64_EncodeBlock(unsigned char *t, const unsigned char *f, int dlen)
	{
	int i,ret=0;
	unsigned long l;
...
int Base64_DecodeFinal(ENCODE_CTX *ctx, unsigned char *out, int *outl)
	...
	*outl=0;
	if (ctx->num != 0)
		{
		i=Base64_DecodeBlock(out,ctx->enc_data,ctx->num);
		if (i < 0) return(-1);
		ctx->num=0;
		*outl=i;
		return(1);
...
#ifdef undef
int Base64_DecodeValid(unsigned char *buf, int len)
	{
	int i,num=0,bad=0;
