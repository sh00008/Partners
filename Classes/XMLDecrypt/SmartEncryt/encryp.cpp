...
{
	if (pInBuffer == NULL|| nLength == 0 || pKey == NULL || nKeyLength == 0 || nIVLength != 16 || dwEncrypt_ModeID != BLOCK_CIPHER_AES_128_AND_CTR || pOutBuffer == NULL) 
	{
		return false;
	}
	...
	unsigned char ivec[AES_BLOCK_SIZE];
	unsigned char ecount_buf[AES_BLOCK_SIZE];
	memcpy(ivec, pIV, nIVLength);
	memset(ecount_buf, 0, sizeof(ecount_buf));

	AES_KEY key;
    if (AES_set_encrypt_key(pKey,nKeyLength*8, &key))
		return false;
	
	outlen = nLength;
	unsigned int num = 0;
	AES_ctr128_encrypt(pInBuffer, pOutBuffer, nLength, &key, ivec, ecount_buf, &num);
					  ...
					  unsigned char *pOutBuffer, long &outlen, int dwEncrypt_ModeID)
{
	...
	if (pInBuffer == NULL|| nLength == 0 || pKey == NULL || nKeyLength == 0 || nIVLength != 16 || dwEncrypt_ModeID != BLOCK_CIPHER_AES_128_AND_CTR) 
	{
		return false;
	}
